...
const QUALITY_COMPARISON_QUESTIONS = [
  ...
  { id: 'oRPz5V', name: 'Visual appeal' },
  { id: 'GpGZaj', name: 'Feature offering' },
  { id: 'GpGbqo', name: 'Professional appeal' },
  { id: 'OX2Eb8', name: 'Community and ecosystem support' }
];

const QualityComparisonRatingsGrid = ({ filters = {}, wasmService }) => {
  ...
  useEffect(() => {
    const fetchAllQuestionData = async () => {
      try {
        ...
        const fetchPromises = QUALITY_COMPARISON_QUESTIONS.map(async (question) => {
  ...
  return (
    <div role="region" aria-labelledby={sectionId} className="w-full bg-white border border-gray-300 rounded-[5px] overflow-hidden shadow-sm" data-chart-id={generateSectionId("Compared to alternatives, Node-RED's quality is ...")}>
      {/* Icon Section */}
      <div className="flex">
        ...
        <div className="flex-1">
          ...
          <div className="px-4 py-3 border-b border-gray-200 bg-white">
            <ChartHeader title="Compared to alternatives, Node-RED's quality is ..." onSectionIdReady={setSectionId} />
          </div>
          <SkipLink chartId={generateSectionId("Compared to alternatives, Node-RED's quality is ...")} label="Skip this section" />

          {/* Questions Grid */}
          <div className="divide-y divide-gray-200">
            {QUALITY_COMPARISON_QUESTIONS.map((question) => {
...
export default QualityComparisonRatingsGrid;