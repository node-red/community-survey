...
const CHANNELS = [
  ...
  { id: '9Zk4d5', name: 'Node-RED Flow Library' },
  { id: 'kGo8b1', name: 'AI Assistance (ChatGPT/etc)' },
  { id: 'a4LpOv', name: 'Node-RED Cookbook' },
  { id: 'WRWqAk', name: 'Community Forum (Discourse)' },
  { id: '6Kl6DP', name: 'GitHub' },
  { id: 'vDZ8Xv', name: 'Stack Overflow' },
  { id: 'erJzBl', name: 'Node-RED Academy' },
  { id: 'BpyJE5', name: 'Reddit' },
  { id: 'bep1Zg', name: 'Discord' },
  { id: '7KA7XP', name: 'Slack' },
  { id: 'ApgMr0', name: 'Facebook Group' }
];

const ChannelRatingsGrid = ({ filters = {}, compact = true, wasmService }) => {
    ...
    const fetchAllChannelData = async () => {
      try {
        ...
        const fetchPromises = CHANNELS.map(async (channel) => {
          const result = await wasmService.getQuantitativeData(channel.id, filters);

          if (result.data && result.data.length > 0) {
            const totalResponses = result.total_respondents || 0;
            const filteredResponses = result.filtered_respondents || totalResponses;
            const processedData = processRatingData(result.data, 7, channel.id);
...
export default ChannelRatingsGrid;